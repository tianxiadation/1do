...
* 
*/
public class BoardTaskServiceA {
	...
	*/
	public static void TaskCalculate(Date date) {
		...
		//List<T1doProject> ts=T1doProject.getAllProjectByDailyReportExists(s.format(date));
		for (T1doProject t : ts) {
	        ...
	        if(sb.length()>0)
	         t1dobase =T1doBase.selectMaxAndMinT1doProject1do(sb.deleteCharAt(sb.length()-1));
			...
			if(t1dobase==null|| t1dobase.getStr("O_FINISH_TIME")==null||t1dobase.getStr("O_CREATE_TIME")==null) {
				...
				holidays=Holiday.dao.find("select * from holiday where item_id=?",t.getItemId()); 
			...
			for (T1doBoard it : items) {
				...
				for (T1doBase tb : list) {
					...
					for (Holiday holiday : holidays) {
						long hl=TimeUtil.getStrToLongTIme(holiday.getDate());
						if(hl>=tb.getOStartTime().getTime()&&hl<=tb.getOFinishTime().getTime()) {
							holiday.setAtemp(holiday.getDay());
							holiday.setDtemp(holiday.getDay());
							holiday.setBtemp(holiday.getDay());
							holiday.setData(holiday.getDay());
						}
						
					  }
					calculation(tb,t,it,k,tt,date,1,holidays);
				
					Holiday h=Holiday.getHolidayData(holidays);
				...
				k=0;
				if(StrUtil.isNotEmpty(str[1])) {
					String[] str11=str[1].split("~");
					for (Holiday holiday : holidays) {
						long hl=TimeUtil.getStrToLongTIme(holiday.getDate());
							if(hl>=TimeUtil.getStrToLongTIme(str11[0])&&hl<=TimeUtil.getStrToLongTIme(str11[1])) {
								holiday.setAtemp(holiday.getDay());
								holiday.setDtemp(holiday.getDay());
								holiday.setBtemp(holiday.getDay());
								holiday.setData(holiday.getDay());
				...
				List<T1doBoardDaliyReport> trs=T1doBoardDaliyReport.dao.find("SELECT DATE FROM `t_1do_board_daliy_report` where TASK_ID=? GROUP BY DATE",it.getID());
				List<String> dates=new ArrayList<String>();
				for (T1doBoardDaliyReport tr : trs) {
					dates.add(TimeUtil.getDate(tr.getDATE()));
				}
				if(dates.size()>0) {
				  for (Holiday holiday : holidays) {
					if(dates.contains(holiday.getDate()))
					 holiday.setData("D");
				  }
				}
				...
				for (T1doBase tb : list) {
					//计算甘特图
					calculation(tb,t,it,k,tt,date,2,holidays);
					
				}				
				//Db.update("update holiday set dtemp='D' where date=? and item_id=? and data not in ('B','C','D')",TimeUtil.getDate(date),t.getItemId());
				for (Holiday holiday : holidays) {				
					if(!holiday.getData().equals("B")&&!holiday.getData().equals("C")&&!holiday.getData().equals("D")&&holiday.equals(TimeUtil.getDate(date))) {						
						holiday.setDtemp("D");
					}
				  }
				
				if(it.getCOMPLETION().equals("已完成")&&it.getFinishDate().getTime()<=date.getTime()) {					
					...
					for (Holiday holiday : holidays) {
						long hl=TimeUtil.getStrToLongTIme(holiday.getDate());
						if(hl>TimeUtil.getStrToLongTIme(TimeUtil.getDate(it.getFinishDate()))) {
							
							holiday.setData("");
						}else if(hl==TimeUtil.getStrToLongTIme(TimeUtil.getDate(it.getFinishDate()))){
							holiday.setData("A");
						}
					}
					...
					for (Holiday holiday : holidays) {
						long hl=TimeUtil.getStrToLongTIme(holiday.getDate());
						if(hl>TimeUtil.getStrToLongTIme(TimeUtil.getDate(date))) {
							
							holiday.setAtemp("");
						}else if(hl==TimeUtil.getStrToLongTIme(TimeUtil.getDate(date))){
							holiday.setAtemp("A");
						}
					}
					Holiday h=Holiday.getHolidayData(holidays);
			...
			//回归初始状态
		    Db.update("update holiday set item_id= null");
			//Db.update("update holiday set data= right(time,2),atemp=right(time,2),dtemp=right(time,2),btemp=right(time,2)");
			for (Holiday holiday : holidays) {
				holiday.setAtemp("");
				holiday.setDtemp("");
		...
		//Db.update("update holiday set data=day,atemp=day,dtemp=day,btemp=day");
	 ...
	 * @param holidays 
	*/
	public static void calculation(T1doBase tb,T1doProject t,T1doBoard it,int k,T1doBoardTask tt,Date date,int type, List<Holiday> holidays) {
		...
		if(tb.getOStatus()<5) {
			...
			//	Db.update("update holiday set data= 'C',atemp='C',dtemp='C',btemp='C'  where date BETWEEN ? and ? and item_id=? and data not in('A','B')",TimeUtil.getDate(tf.get(0).getFbTime()),str,t.getItemId());
				for (Holiday holiday : holidays) {
					long hl=TimeUtil.getStrToLongTIme(holiday.getDate());
					if(!holiday.getData().equals("A")&&!holiday.getData().equals("B")&&hl>=TimeUtil.getStrToLongTIme(TimeUtil.getDate(tf.get(0).getFbTime()))&&hl<=TimeUtil.getStrToLongTIme(str)) {
						holiday.setAtemp("C");
						holiday.setDtemp("C");
						holiday.setBtemp("C");
						holiday.setData("C");
				...
				//	Db.update("update holiday set data= 'B',atemp='B',dtemp='B',btemp='B'  where date BETWEEN ? and ? and item_id=? and data not in('A')",TimeUtil.getDate(tf.get(4).getFbTime()),str,t.getItemId());
					for (Holiday holiday : holidays) {
						long hl=TimeUtil.getStrToLongTIme(holiday.getDate());
						if(!holiday.getData().equals("A")&&hl>=TimeUtil.getStrToLongTIme(TimeUtil.getDate(tf.get(4).getFbTime()))&&hl<=TimeUtil.getStrToLongTIme(str)) {
							holiday.setAtemp("B");
							holiday.setDtemp("B");
							holiday.setBtemp("B");
							holiday.setData("B");
			...
			if(flag) {
				...
				for (Holiday holiday : holidays) {
					long hl=TimeUtil.getStrToLongTIme(holiday.getDate());
					if(!holiday.getData().equals("A")&&!holiday.getData().equals("B")&&hl>=TimeUtil.getStrToLongTIme(TimeUtil.getDate(TimeUtil.getNextDay(tb.getOFinishTime(),1)))&&hl<=TimeUtil.getStrToLongTIme(TimeUtil.getDate(date))) {
						holiday.setAtemp("C");
						holiday.setDtemp("C");
						holiday.setBtemp("C");
						holiday.setData("C");
				...
				if(TimeUtil.getBetweenDays1(TimeUtil.getDate(tb.getOFinishTime()),TimeUtil.getDate(date))>4) {
					...
					for (Holiday holiday : holidays) {
						long hl=TimeUtil.getStrToLongTIme(holiday.getDate());
						if(!holiday.getData().equals("A")&&hl>=TimeUtil.getStrToLongTIme(TimeUtil.getDate(TimeUtil.getNextDay(tb.getOFinishTime(),5)))&&hl<=TimeUtil.getStrToLongTIme(TimeUtil.getDate(date))) {
							holiday.setAtemp("B");
							holiday.setDtemp("B");
							holiday.setBtemp("B");
							holiday.setData("B");
          ...
          //超时了,修改拟完成时间到完成时间为闪电
			if(flag) {
				...
				for (Holiday holiday : holidays) {
					long hl=TimeUtil.getStrToLongTIme(holiday.getDate());
					if(!holiday.getData().equals("A")&&!holiday.getData().equals("B")&&hl>=TimeUtil.getStrToLongTIme(TimeUtil.getDate(TimeUtil.getNextDay(tb.getOFinishTime(),1)))&&hl<=TimeUtil.getStrToLongTIme(TimeUtil.getDate(tb.getRealFinishTime()))) {
						holiday.setAtemp("C");
						holiday.setDtemp("C");
						holiday.setBtemp("C");
						holiday.setData("C");
				...
				if(TimeUtil.getBetweenDays1(TimeUtil.getDate(tb.getOFinishTime()),TimeUtil.getDate(date))>4) {
					...
					for (Holiday holiday : holidays) {
						long hl=TimeUtil.getStrToLongTIme(holiday.getDate());
						if(!holiday.getData().equals("A")&&hl>=TimeUtil.getStrToLongTIme(TimeUtil.getDate(TimeUtil.getNextDay(tb.getOFinishTime(),5)))&&hl<=TimeUtil.getStrToLongTIme(TimeUtil.getDate(tb.getRealFinishTime()))) {
							holiday.setAtemp("B");
							holiday.setDtemp("B");
							holiday.setBtemp("B");
							holiday.setData("B");
			...
			//	Db.update("update holiday set data= 'A',atemp='A'  where date=? and item_id=?",TimeUtil.getDate(tb.getRealFinishTime()),t.getItemId());
				...
				for (Holiday holiday : holidays) {
					long hl=TimeUtil.getStrToLongTIme(holiday.getDate());
					if(hl>TimeUtil.getStrToLongTIme(TimeUtil.getDate(tb.getRealFinishTime()))) {
						holiday.setAtemp("");
						holiday.setData("");
					}else if(hl==TimeUtil.getStrToLongTIme(TimeUtil.getDate(tb.getRealFinishTime()))){
						holiday.setAtemp("A");
						holiday.setData("A");
					}
				}
	...
	*/
	public static void temp() {
		List<T1doBoardTask> tt=T1doBoardTask.dao.find("select * from t_1do_board_task WHERE DATE='2019-11-13' group by Date");//'
