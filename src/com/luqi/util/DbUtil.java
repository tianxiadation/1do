...
public class DbUtil {
	    public static void saveWeight1(String SHOW_ID){
			Db.update("insert into t_1do_label(SHOW_ID,LABEL)"
			+" SELECT a.SHOW_ID,a.LABEL FROM t_1do_label a,t_1do_weight b where a.LABEL=b.LABEL and a.SHOW_ID=?"
			,SHOW_ID);
		}
	...
	*/
	...
	Db.update("insert into t_1do_relation(SHOW_ID,RELATION_SHOW_ID,SIMILARITY,TYPE) "
			  +" select a.RELATION_SHOW_ID,a.SHOW_ID,0,b.O_STATUS from t_1do_relation a,t_1do_base b "
			  +" where a.SHOW_ID=? and a.SHOW_ID=b.SHOW_ID and a.SIMILARITY=0",SHOW_ID);
	}
	/*
	...
	*/
	public static void updateType(String showid) {
		//排序 SORT-1是批量添加的关联 -2父1do -3是子1do    type(0手动添加的关联。6已删除的1do，-1父1do，-2子1do)
	...
	*/
	public static int update(String showid,String showid1,int type,int temp,String showid2,String showid3) {
		...
		case 1:			
			//删除1do关联
			return Db.update("update t_1do_relation a,t_1do_base b set a.sort=0,a.TYPE=7 "
					+ "where ((a.SHOW_ID=? and a.RELATION_SHOW_ID=?) or (a.SHOW_ID=? and a.RELATION_SHOW_ID=?)) and b.SHOW_ID=a.RELATION_SHOW_ID "
		...
		case 5:
			return Db.update("update t_1do_pstatus set isRead=1,urge_isLook=1 where SHOW_ID=? and O_USER=? and isDelete=1 and USER_TYPE!=2 and (online=2 or gmt_modified<CURDATE())",showid,showid1);
		case 6:	
			return Db.update("update t_1do_fwpstatus set isRead=1 where SHOW_ID=? and O_USER=? and (online=2 or gmt_modified<CURDATE())",showid,showid1);
		case 7:
			//在线已读
			Db.update("update t_1do_pstatus set online=2  where SHOW_ID!=? and O_USER=?",showid,showid1);
			Db.update("update t_1do_pstatus set online=1,isRead=1,urge_isLook=1 where SHOW_ID=? and O_USER=?",showid,showid1);
			Db.update("update t_1do_fwpstatus set online=1,isRead=1 where SHOW_ID=? and O_USER=?",showid,showid1);
			return 1;
		case 8:		
			//修改创建用户
		...
		case 15:
			...
			//Db.update("update t_1do_pstatus set isRead=2  where  (online=2 or gmt_modified<CURDATE()) and SHOW_ID=? and (USER_TYPE=1 or O_USER=? ) and isDelete=1",showid,showid1);
			Db.update("update t_1do_pstatus set isRead=2  where  (online=2 or gmt_modified<CURDATE()) and SHOW_ID=?  and isDelete=1",showid);
			Db.update("update t_1do_fwpstatus set isRead=2 where SHOW_ID=? and (online=2 or gmt_modified<CURDATE()) ",showid);
			
			return 1;
		case 16:	
			//反馈设置用户未读
			Db.update("update t_1do_pstatus set isRead=2 where SHOW_ID=? and isDelete=1 and (online=2 or gmt_modified<CURDATE())",showid);
			Db.update("update t_1do_fwpstatus set isRead=2 where SHOW_ID=? and (online=2 or gmt_modified<CURDATE())",showid);
			return 1;
		case 17:			
			return 1;
	...
	public static void deleteALLLable(String SHOW_ID) {
		Db.delete("delete from t_1do_label where SHOW_ID=?",SHOW_ID);
		
	}
	public static void updateUser(String[] str) {
		Db.update("update t_reg_user set LAST_LOGIN_TOKEN=?,LAST_UPDATE_TIME=now() where SHOW_ID=?",str[0],str[1]);
