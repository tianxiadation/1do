...
public class T1doFeedback extends BaseT1doFeedback<T1doFeedback>  {
	...
	*/
	public T1doSet getIdoSet() {
		return T1doSet.dao.findFirst("select * from t_1do_set where SHOW_ID=? and EVENT_TYPE like ?",get("SHOW_ID"), "%"+get("FB_TYPE")+"%");
	}
	/*
	 2018年6月25日下午9:16:58 方升群
	*/
	public T1doPset getIdoPset() {
		return T1doPset.dao.findFirst("select * from t_1do_pset where SHOW_ID=? and O_USER=? and EVENT_TYPE like ?",get("SHOW_ID"),get("O_USER"),"%"+get("FB_TYPE")+"%");
	}
	/*
	 2018年6月25日下午9:11:15 方升群  //查询是否该工单不可以反馈
	 */
	public T1doSet getIdoSet1() {
		return T1doSet.dao.findFirst("select * from t_1do_set where SHOW_ID=? and EVENT_TYPE like '%3%'",get("SHOW_ID"));
	}
	/*
	 2018年6月25日下午9:16:58 方升群   //查询是否该用户对该工单不可以反馈
	 */
	public T1doPset getIdoPset1() {
		return T1doPset.dao.findFirst("select * from t_1do_pset where SHOW_ID=? and O_USER=? and EVENT_TYPE like '%3%'",get("SHOW_ID"),get("O_USER"));
	}
	/*
	...
	*/
	...
	public T1doBase getT1doBase() {
		...
		return T1doBase.dao.findFirst("select * from t_1do_base where SHOW_ID=?", get("SHOW_ID"));
	}
	/*
