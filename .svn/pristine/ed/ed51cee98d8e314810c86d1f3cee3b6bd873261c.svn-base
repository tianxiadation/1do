...
import com.luqi.common.model.base.BaseT1doBase;
...
public class T1doBase extends BaseT1doBase<T1doBase> {
	...
	//反馈查询的字段
	public static String selectField ="select ID,O_USER_NAME,TIME_STAMP,FB_TIME,O_USER,FBCONTENT,FB_TYPE,FB_USER_NAME,FB_USER,ATTR_PATH,star,AT,IS_CREATE_REPORT";
	//主动工单标题
	public static String[] title= {"","城管执法","民政","环保","住建","人社","应急","文化","市场"};
	
	/*
	 2019年4月30日 coco 注解：保存1do工单
	*/
	public void saveBase(JSONObject user,String showId,int files) {
		this.setOCreateTime(new Date()).setCreateUser(user.getString("loginName"))
		.setCreateUserName(user.getString("username")).setShowId(showId).setSendTime(new Date().getTime())
		.setFBNUM(files);
		
		if(StrKit.notBlank(this.getOTypeId())&&this.getSOURCE()!=null&&this.getSOURCE()==8){
			if(Integer.valueOf(this.getOTypeId())>0&&Integer.valueOf(this.getOTypeId())<9)
			this.setOTitle(title[Integer.valueOf(this.getOTypeId())]).save();
			else 
			this.setOTitle("主动办").save();
		}else if(this.getSOURCE()!=null&&this.getSOURCE()==11) {
			if(StrUtil.isEmpty(this.getOTitle()))
				this.setOTitle("云上城管");
			this.save(); 
		}else{
			this.setOTitle(this.getODescribe().substring(0,this.getODescribe().length()>30?30:this.getODescribe().length())).save(); 
		}
	...
	/*
	 2019年4月30日 coco 注解：保存1do工单
	*/
	public void saveBase(String showId,int files) {
		this.setOCreateTime(new Date())
		.setShowId(showId).setSendTime(new Date().getTime()).setFBNUM(files) ;
		if(this.getSOURCE()==null) {
			this.setSOURCE(1);
		} 
		if(this.getSOURCE()!=11){
			this.setOTitle(this.getODescribe().substring(0,this.getODescribe().length()>30?30:this.getODescribe().length()));
		}else{
			this.setOTitle("云上城管");
		}
		this.save();
	...
	/*
	 2019年4月26日 coco 注解：修改查看数和look_user
	*/
	public void updateLookNumAndLookUser(String loginName) {
		this.setLOOKNUM(this.getLOOKNUM()+1);
		if(StrKit.isBlank(this.getLookUser())){
			this.setLookUser(loginName);
		}else if(!this.getLookUser().contains(loginName)){
			this.setLookUser(this.getLookUser()+","+loginName);
		}
		this.update();
	}
	/*
	...
	*/
	public static List<T1doBase> selectBybase(JSONObject json) {
		String str="select  b.SHOW_ID from t_1do_relation a,t_1do_base b "
				+ "where (a.SIMILARITY>b.SIMILARITY or a.TYPE<=0) and a.RELATION_SHOW_ID=b.SHOW_ID and a.SHOW_ID=? ";
		return T1doBase.dao.find("select SHOW_ID,O_DESCRIBE,O_CREATE_TIME "
				+ "from t_1do_base where O_STATUS!=6 and O_DESCRIBE LIKE CONCAT('%','"+json.getString("BASE")+"','%') and "
						+ "SHOW_ID not in ("+str+") and SHOW_ID!=?",json.getString("SHOW_ID"),json.getString("SHOW_ID"));
		//select RELATION_SHOW_ID  from t_1do_relation where SHOW_ID=? and (SIMILARITY>10 or type=0)
	...
	*/
	...
	/*
	 2018年6月26日下午10:54:30 方升群  //获得不同状态的反馈消息
	*/
	public List<T1doFeedback> getIdoFeedbacks1(int id,int num, String sql) {
		return T1doFeedback.dao.find("select * from ("+selectField+" from t_1do_feedback where SHOW_ID=? and FB_TYPE!=4 and Fb_TYPE!=8 and Fb_TYPE!=7 "+sql+" and isoverdue=1 ORDER BY ID DESC LIMIT ?,?)a ORDER BY ID",getShowId(),id,num);
	}
	
	public List<T1doFeedback> getIdoFeedbacks1(int id,int num) {
		return T1doFeedback.dao.find("select * from ("+selectField+" from t_1do_feedback where SHOW_ID=? and FB_TYPE!=4 and Fb_TYPE!=8 and Fb_TYPE!=7 and isoverdue=1 ORDER BY ID DESC LIMIT ?,?)a ORDER BY ID",getShowId(),id,num);
	}
	...
	*/
	...
	public static T1doBase getIdoBase2(String SHOW_ID) {
		return T1doBase.dao.findFirst("select ID,O_TITLE,SHOW_ID,O_DESCRIBE,O_CUSTOMER,"
				+ "O_CUSTOMER_NAME,O_START_TIME,O_FINISH_TIME,O_EXECUTOR,O_EXECUTOR_NAME,"
				+ "O_IS_DELETED,O_CREATE_TIME,Real_FINISH_TIME,DELETE_TIME,LOOKNUM,AT,O_STATUS,LOOK_USER,"
				+ "SOURCE,O_TYPE_ID,WECHAT_GROUP_NAME,WECHAT_GROUP_ID,IS_BINDING_WECHAT_GROUP "
				+ "from t_1do_base where SHOW_ID=?", SHOW_ID);
	}
	public static T1doBase getIdoBase3(String SHOW_ID) {
		return T1doBase.dao.findFirst("select SHOW_ID,O_DESCRIBE,O_STATUS,SOURCE from t_1do_base where SHOW_ID=?", SHOW_ID);
	}
	/*public static T1doBase getIdoBase2(String SHOW_ID) {
		return T1doBase.dao.findFirst("select * from t_1do_base where SHOW_ID=?", SHOW_ID);
	}*/
	...
	public T1doPset getIdoPset1() {
		return T1doPset.dao.findFirst("select * from t_1do_pset where SHOW_ID=? and O_USER=? and EVENT_TYPE like '%2%'",getShowId(),get("O_RANGE"));
	}
	...
	*/
	public int setUserotherid(JSONObject json) {
		...
		public  List<T1doPstatus> getUser(int i) {
			if(i<3){
			    return T1doPstatus.dao.find("select O_USER,O_USER_NAME from t_1do_pstatus where SHOW_ID=? and otherid =? and isDelete=1 GROUP BY SHOW_ID,O_USER",getShowId(),i);
		    }else{
		    	return T1doPstatus.dao.find("select O_USER,O_USER_NAME from t_1do_pstatus where SHOW_ID=? and USER_TYPE =? and isDelete=1 GROUP BY SHOW_ID,O_USER order by otherid",getShowId(),i-2);
		    }
		...
		*/
		...
		　* 描述：   根据节点id查询关联1do
		　* 创建人：coco   
		　* 创建时间：2019年8月22日 上午10:01:32         
		*/
		public static List<T1doBase> selectT1doProject1do(Long id) {
			return dao.find("SELECT b.* FROM t_1do_project_1do a,t_1do_base b where a.SHOW_ID=b.SHOW_ID and a.ITEM_ID=? and b.O_FINISH_TIME is not null and b.O_STATUS<6",id);
		}
		/**   
		　* 描述：   根据所有节点id查询所有关联1do
		　* 创建人：coco   
		　* 创建时间：2019年8月22日 上午10:01:32         
		*/
		public static T1doBase selectAllT1doProject1do(StringBuilder deleteCharAt) {
			 return dao.findFirst("SELECT min(O_CREATE_TIME) O_CREATE_TIME,max(O_FINISH_TIME)  O_FINISH_TIME FROM t_1do_project_1do a,t_1do_base b where a.SHOW_ID=b.SHOW_ID and a.ITEM_ID in(?) and b.O_FINISH_TIME is not null",deleteCharAt);
		}
		/**   
		 * 描述：   根据所有节点id查询所有关联1do
		 * 创建人：coco   
		 * 创建时间：2019年8月22日 上午10:01:32         
		 */
		public static Record selectMaxAndMinT1doProject1do(StringBuilder deleteCharAt) {
			return Db.findFirst("SELECT left(min(O_START_TIME),10) O_CREATE_TIME,left(max(O_FINISH_TIME),10)  "
					+ "O_FINISH_TIME FROM t_1do_project_1do a,t_1do_base b where a.SHOW_ID=b.SHOW_ID "
					+ "and a.ITEM_ID in("+deleteCharAt+") and b.O_FINISH_TIME is not null");
		}
		//修改计划时间
		public void changePlannedTime(JSONObject json) {
			Db.update("update t_1do_base set O_FINISH_TIME=?,O_START_TIME=? where SHOW_ID=?",json.getString("O_FINISH_TIME"),json.getString("O_START_TIME"),json.getString("SHOW_ID"));
			
		}
		//修改工单状态
		public static int updateStatus(int O_STATUS, String SHOW_ID) {
			if(O_STATUS<6)
			return 	 Db.update("update t_1do_base set O_IS_DELETED=1 ,DELETE_TIME=null,O_STATUS=? where SHOW_ID=?",O_STATUS,SHOW_ID);
			else
			return Db.update("update t_1do_base set O_IS_DELETED=2 ,DELETE_TIME=now(),O_STATUS=? where SHOW_ID=?",O_STATUS,SHOW_ID);	  
		}
		//初始化工单
		public static void initialization(String SHOW_ID) {
			Db.update("update t_1do_base set O_IS_DELETED=1 ,SEND_TIME=?,O_STATUS=3,LOOKNUM=0,FBNUM=0,LIGHTNING=0 where SHOW_ID=?",new Date().getTime(),SHOW_ID);	

			
		}

		public static void main(String[] args) {
			System.out.println("ssssssssddddddddaasasafafa".substring(0,"ssssssssddddddddaasasafafa".length()));
		}
